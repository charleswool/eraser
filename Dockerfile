...
# HostProcess base: about 22 kB, because the process runs on the host and the
# image supplies no OS of its own.
...
# A HostProcess container runs on the host, so an entrypoint of /collector would
# resolve against the host root. The image's files only exist under the sandbox
